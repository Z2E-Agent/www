...
The TUI is built with the **Bubble Tea** framework (`github.com/charmbracelet/bubbletea`) and lives in `internal/ui/model.go` (~1,333 LOC).

## Component Tree

- **AppModel**
  - **Transcript** (viewport.Model) - rendered markdown via glamour
  - **Composer** (textarea.Model) - input area, sends on Enter
  - **StatusBar** (lipgloss) - model name, connection state, step counter, mode
  - **ModelPicker** (list.Model) - `/model` command overlay
  - **RuntimeBridge** (goroutine) - stream events to `tea.Cmd`
...
- Shows:
  - Current model name (e.g., `opencode-go/deepseek-v4-pro`)
...
- Shows available models from the catalog, grouped by provider
- `/model <provider>/<model>` switches directly
- `/model <provider>` lists that provider's models
- Selection updates the active model at runtime (no restart)

## Streaming Pipeline

SSE chunks travel through a 6-stage pipeline:

<Mermaid chart={`flowchart LR
    A[SSE Chunk] --> B[Gateway Client] --> C[Stream Bridge] --> D[TUI Update] --> E[Lipgloss Render] --> F[Terminal]
`} />

1. **SSE Chunk**: Raw `data:` lines from the OpenCode Go gateway
