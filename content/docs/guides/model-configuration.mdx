...
Gateway calls retry on failure with exponential backoff:
...
Only gateway transport errors trigger retries (not model-level errors or tool execution failures).
